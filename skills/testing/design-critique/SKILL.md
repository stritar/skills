...
- **Code or markup file** (HTML, JSX/TSX, Vue, Svelte, a CSS+HTML pair, etc.): **render it, then
  critique the pixels.** Use the renderer bundled with this skill. `<skill-dir>` is this skill's
  own directory (the folder containing this SKILL.md), and the scripts live in `<skill-dir>/scripts`:
