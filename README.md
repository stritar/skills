...
The documentation site in [site/](site/) is built with
[Fumadocs](https://fumadocs.dev) from the skill files themselves. It has a
filterable directory of every skill and a page per skill. Each page carries
the full instructions, the catalog metadata and the supporting files.
Full-text search (⌘K or Ctrl K) runs in the browser, with no search service.
Every page links to its raw `SKILL.md`, and `/llms.txt` lists all skills with
links to their raw files.

No skill text is copied into the site by hand. On each build,
`scripts/lib/docs-content.mjs` reads `skills/` and `catalog/index.json`, so a
new or changed skill appears on the next build. The directory filter uses the
same ranking code as the CLI.

Run it locally with Node 22 or later:

```bash
cd site
npm install
npm run dev            # http://localhost:3000
npm run build          # static export in site/out
npm run check:export   # pages, links, raw files and llms.txt in the export
npm run preview        # serves site/out the way GitHub Pages does
```

[.github/workflows/docs-site.yml](.github/workflows/docs-site.yml) builds and
checks the site on every pull request and deploys `main` to GitHub Pages. It
needs the Pages source set to "GitHub Actions" in the repository settings.

`docs/index.html` is the older edition of the catalog, generated by
`npm run catalog:build` with no dependencies. It still works straight from
disk.

## Catalog statistics (2026-09-11)

106 skills: 98 vendored third-party, 8 original. 53 are marked as
recommended defaults; 36 are experimental; 2 remain in draft status pending
